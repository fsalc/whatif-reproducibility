...
def execute_gprom(file, database, method):
    #print(f'{GPROM_LOCATION} -backend postgres -host localhost -port {PORT} -user whatif -passwd mahif -db {database} {GPROM_FLAGS} {methods[method]} -queryFile {file}')
    #return check_output(f'{GPROM_LOCATION} -backend postgres -host localhost -port {PORT} -user whatif -passwd mahif -db {database} {GPROM_FLAGS} {methods[method]} -queryFile {file}', stderr=STDOUT, timeout=60*30)
